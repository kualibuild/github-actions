...
if [[ "$1" == "true" ]]; then
  ...
  [[ "$4" =~ ^(\"([0-9]{1,3}.){3}[0-9]\/[0-9]{1,2}\",*){1,}$ ]] || { echo "ERR: original_ips should be one or more ip addresses in CIDR notation quote wrapped and comma delimited"; echo "${USAGE}"; exit 1; }
else

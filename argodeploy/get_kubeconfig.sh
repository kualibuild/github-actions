...
# validate inputs
USAGE="usage: ./get_kubeconfig.sh [true|false] [cluster_name] [region] (Optional: [statefile])"
if [[ $# -lt 2 ]]; then
  echo "${USAGE}"
  exit 1
fi
...
if [[ "$1" == "true" ]]; then
  ...
  [[ "$4" =~ ^s3://.*$ ]] || { echo "ERR: statefile should be an s3 address"; echo "${USAGE}"; exit 1; }
fi
...
if [[ "$1" == "false" ]]; then
  ...
  # configure kubectl 
  kubectl config set-context ${2} --cluster=${2} --user=${2}
